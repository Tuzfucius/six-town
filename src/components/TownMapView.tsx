...
export default function TownMapView() {
  ...
  return <main className="relative h-screen overflow-hidden bg-[#080b12] text-white">
    ...
    <header className="pointer-events-none absolute left-4 right-4 top-4 z-20 flex items-start justify-between gap-4 sm:left-6 sm:right-6 sm:top-6">
      ...
      <div className="pointer-events-auto hidden items-center gap-2 rounded-xl border border-white/15 bg-[#0b111c]/90 px-3 py-2 text-xs text-white/65 shadow-xl backdrop-blur sm:flex"><Rotate3D className="h-4 w-4 text-cyan-100" /><span>拖拽平移，右键拖拽或控制器旋转视角</span></div>
    </header>

    <aside className={`absolute bottom-4 left-4 top-20 z-20 flex overflow-hidden rounded-2xl border border-white/15 bg-[#0b111c]/95 shadow-2xl backdrop-blur-xl transition-[width] duration-200 sm:bottom-6 sm:left-6 sm:top-24 ${collapsed ? 'w-12' : 'w-[min(22rem,calc(100vw-2rem))]'}`} aria-label="企业概要列表">
